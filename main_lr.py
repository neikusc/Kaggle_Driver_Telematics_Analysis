...
def create_attributes(fileId):
  ...
  speed = np.sqrt(vel_xy.x**2 + vel_xy.y**2)
  ...
  velocities = np.array(speed)
  accelerations = list(accelerations)
  speed_up = (velocities[1:]-velocities[:-1]>0)*velocities[1:]
  speed_down = (velocities[1:]-velocities[:-1]<0)*velocities[1:]
  
  standing_time = sum(v < 0.01 for v in velocities)
  standing_speed = sum(a < 0.001 for a in accelerations)
  
  avg_speed = np.mean(velocities)
  avg_speed_up = np.mean(speed_up)
  avg_speed_down = np.mean(speed_down)
  ...
  attributes = [driverId+'_'+tripId, trip_time, total_distance, skyway_distance, avg_speed, std_speed, 
                avg_speed_up, avg_speed_down, avg_acc, std_accel,
...
if __name__ == '__main__':
  ...
  data = pd.DataFrame.from_csv('training_set.csv')
